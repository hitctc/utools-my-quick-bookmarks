...
- 当前真实能力：
  ...
  - 通过 uTools 顶部输入框按空格分词做多关键词 `AND` 搜索，并高亮标题、域名和目录路径中的命中片段；不再搜索完整 URL 路径和参数
  - 书签标题额外支持全拼和拼音首字母搜索
  - 支持上下左右方向键按可见卡片空间位置切换高亮项（左右键可跨列移动且不移动顶部输入框光标）、自动滚动保持当前项可见、回车打开书签、点击卡片打开书签
  - 支持插件内置顶、最近打开、打开次数
  ...
  - 路径配置、主题模式、窗口高度、置顶、最近打开和打开次数都通过 uTools 数据库持久化；用户在另一台设备同步后，插件会重新读取这些配置
- 当前明确限制：
...
- `src/App.vue`
  当前插件 UI 总入口，负责首页 / 设置页双视图切换、uTools 顶部输入搜索、高亮索引、本地状态整合、主题模式解析和窗口高度应用。
- `src/bookmarks/HomeView.vue`
  首页视图，负责底部悬浮状态 dock、悬浮设置按钮、底部操作提示和书签卡片分区。
...
- 当前书签数据已经做到“读取 + 解析 + 紧凑卡片展示 + 多关键词顶部输入搜索 + 插件内状态”；如果继续加多浏览器探测或任何写回 Chrome 的能力，要同步更新本文档里的阶段定义和已知限制。
...
12. 按上下左右方向键切换高亮卡片，左右键可跨列移动且不会移动顶部输入框光标，确认当前项会自动滚动到可见区域，并且按回车能直接打开
13. 在设置页修改插件窗口高度，确认窗口高度会立即变化；关闭并重新进入插件后，确认仍保持为上次保存值
14. 每次进入插件时，确认会先看到轻量首屏壳子而不是纯白空屏；如果本机已有缓存，再确认首页优先显示上次缓存结果
15. 点击首页右下角刷新按钮，确认状态条显示“刷新中”；如果刷新失败，确认当前列表不被清空
...
9. 在顶部输入框输入一个或多个关键字，确认首页卡片会按空格分词做 `AND` 筛选
...
14. 按上下左右方向键移动高亮项，左右键可跨行/跨列移动且不会移动顶部输入框光标；上下键按视觉相邻行移动；确认当前卡片会保持可见，按回车确认书签可打开
15. 在设置页拖动窗口高度滑块，确认提示文案位于“高度”标签下方，窗口高度会立即变化；点击“恢复默认高度”后确认高度立即回到默认值；关闭并重新进入插件后，确认仍保持为上次保存值
16. 点击卡片右上角置顶按钮，确认卡片会进入“置顶”区块
17. 在设置页切换 `跟随系统 / 深色 / 浅色`，确认首页主题和状态标签会即时更新
18. 关闭并重新进入插件，确认主题模式与首页主题状态可以正确持久化
19. 打开几个书签后回到首页，确认“最近打开”和“打开次数”表现符合设置项
20. 故意改成错误路径，确认设置页原地显示错误，且不会把错误路径写入本地设置
21. 点击“恢复默认路径”，确认可回到默认路径
...
- 路径配置、主题模式、窗口高度和首页 UI 开关都只通过 `utools.dbStorage` 保存；书签秒开缓存单独落在本机缓存文件，不参与 uTools 数据同步。
...
- 当前已具备：
  ...
  - `跟随系统 / 深色 / 浅色` 三态主题
  - 主插件窗口高度设置与持久化
  - 顶部输入多关键词 `AND` 搜索、标题全拼 / 拼音首字母搜索与命中高亮
  - 插件内置顶 / 最近打开 / 打开次数
