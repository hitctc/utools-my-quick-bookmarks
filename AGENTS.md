...
- 当前真实能力：
  ...
  - 支持在设置页修改主插件窗口高度，并通过 `dbStorage` 持久化到用户同步数据
- 当前明确限制：
...
- `src/bookmarks/SettingsView.vue`
  设置页视图，负责修改路径、恢复默认路径、刷新书签、主题模式、窗口高度、UI 开关、原首页迁移来的说明信息，以及悬浮返回按钮。
...
11. 按上下方向键切换高亮卡片，确认当前项会自动滚动到可见区域，并且按回车能直接打开
12. 在设置页修改插件窗口高度，确认窗口高度会立即变化；关闭并重新进入插件后，确认仍保持为上次保存值
...
10. 确认标题、域名、目录路径命中会高亮；仅隐藏 URL 命中时会显示“网址命中”
11. 按上下方向键移动高亮项，确认当前卡片会保持可见，按回车确认书签可打开
13. 点击卡片右上角置顶按钮，确认卡片会进入“置顶”区块
14. 在设置页切换 `跟随系统 / 深色 / 浅色`，确认首页主题和状态标签会即时更新
15. 关闭并重新进入插件，确认主题模式与首页主题状态可以正确持久化
16. 打开几个书签后回到首页，确认“最近打开”和“打开次数”表现符合设置项
17. 故意改成错误路径，确认设置页原地显示错误，且不会把错误路径写入本地设置
18. 点击“恢复默认路径”，确认可回到默认路径
...
- 路径配置、主题模式、窗口高度和首页 UI 开关都只通过 `utools.dbStorage` 保存，避免额外落地本地明文配置文件。
