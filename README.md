...
- 设置页支持 `跟随系统 / 深色 / 浅色` 三态主题切换
- 首页会根据主题模式切换 `Nothing` 风格的深浅色界面
...
- 当前主题字体使用本地优先字体栈，不依赖运行时下载远程字体资源
...
如果改动涉及首页样式或设置页主题切换，除了 `npm test` 之外，至少再跑一次 `npm run build`，并手动检查三态主题切换与持久化是否正常。
