...
- `src/bookmarks/HomeView.vue`
  首页视图，展示当前路径、读取状态、主题状态、搜索提示和书签卡片分区。
- `src/bookmarks/SettingsView.vue`
  设置页视图，负责修改路径、恢复默认路径、刷新书签、主题模式和 UI 开关。
...
12. 在设置页切换 `跟随系统 / 深色 / 浅色`，确认首页主题和状态标签会即时更新
13. 关闭并重新进入插件，确认主题模式与首页主题状态可以正确持久化
14. 打开几个书签后回到首页，确认“最近打开”和“打开次数”表现符合设置项
15. 故意改成错误路径，确认设置页原地显示错误，且不会把错误路径写入本地设置
16. 点击“恢复默认路径”，确认可回到默认路径
