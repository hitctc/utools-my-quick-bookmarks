...
- `public/preload/services.js`
  负责桥接 `utools.dbStorage`、本地文件读取和错误整理，并通过 `window.services` 暴露给前端。
- `src/App.vue`
  当前插件 UI 总入口，负责首页 / 设置页双视图切换，以及书签初始化加载流程。
- `src/bookmarks/HomeView.vue`
  首页视图，展示当前路径、读取状态、错误和解析后的书签列表。
- `src/bookmarks/SettingsView.vue`
  设置页视图，负责修改路径、恢复默认路径和重新读取。
- `src/main.js`
  Vue 客户端挂载入口。
- `src/main.css`
  当前书签工具 UI 的基础样式。
...
- 当前书签数据只做到“读取 + 解析 + 展示”。如果开始加搜索、排序、打开网址或多浏览器探测，要同步更新本文档里的阶段定义和已知限制。
...
9. 故意改成错误路径，确认设置页原地显示错误
10. 点击“恢复默认路径”，确认可回到默认路径
