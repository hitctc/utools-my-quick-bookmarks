...
- `src/App.vue`
  当前插件 UI 总入口，负责首页 / 设置页双视图切换、uTools 顶部输入搜索、高亮索引、本地状态整合和主题模式解析。
- `src/bookmarks/HomeView.vue`
  首页视图，负责薄状态条、悬浮设置按钮、底部操作提示和书签卡片分区。
- `src/bookmarks/SettingsView.vue`
  设置页视图，负责修改路径、恢复默认路径、刷新书签、主题模式、UI 开关和原首页迁移来的说明信息。
...
12. 点击卡片右上角置顶按钮，确认卡片会进入“置顶”区块
13. 在设置页切换 `跟随系统 / 深色 / 浅色`，确认首页主题和状态标签会即时更新
14. 关闭并重新进入插件，确认主题模式与首页主题状态可以正确持久化
15. 打开几个书签后回到首页，确认“最近打开”和“打开次数”表现符合设置项
16. 故意改成错误路径，确认设置页原地显示错误，且不会把错误路径写入本地设置
17. 点击“恢复默认路径”，确认可回到默认路径
